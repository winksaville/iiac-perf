...
Adopt zc-ring-x1's set whole, taking its copy rather than vc-x1's, since the one line they differ
on is zc-ring-x1's own accepted counter. Convert the config to the markdown carrier so
`validate-config` passes and the `[family]` and `[validate]` tables exist, dissolve
`custom-family.md` into the set and that config, reshape `TODO.md` to the set's Todo format, and
repoint or fix every anchor the two validators name. Obvious holes get filled on the way, the
rest becomes a proposal once we have used the set.
...
two validators name eight anchor problems and one broken link across the live files.
...
1. Crossbeam baselines for the unbounded-queue comparison: `cb-chan-1t` / `cb-chan-2t` over
   `crossbeam::channel::unbounded()` and `cb-seg-1t` / `cb-seg-2t` over
   `crossbeam::queue::SegQueue`, so zc-ring-x1's forthcoming segmented SPSC has ecosystem
...
2. Vyukov's unbounded SPSC and zc-ring-x1's SPSC v1, the two implementations the crossbeam
   baselines exist to frame (wink, 2026-08-28)
...
4. Cold-wake profile: measure the post-wake transient (C-state exit, cache and TLB refill, the
   ...
   - same family, opposite sign: reproducibility runs may want deep C-states forbidden (the
     ...
     allowed
5. Rethink environment rating, then resume the qualify power-policy rung: the settle-cell
...
6. Config keys stay CLI-settable: adopt the convention that every run-parameter config key has a
...
7. Prepare for expected errors: before a command whose outcome is not clean (a rebase across
...
8. Blocks as the first-class mode: knobs, always-on error bars, then a measured default flip
...
9. Always work on a topic bookmark: cycles happen on a bookmark, `main` advances only by landing
...
10. Sync the 20260803 agent-files baseline [[84]]
...
11. Qualification reports evidence, not verdicts: retire the prejudging NOT QUALIFIED stamp
...
12. Seam-clock attribution: sample `cpuinfo_avg_freq` at batch seams (the reader exists,
...
13. Qualify the environment without a bench. `qualify-environment` respawns children running
...
14. Guard `--pin` pools smaller than the bench's thread placements, and deadline the estimate
...
15. Move the batch seam's work off the measuring thread, using the FastForward-style SPSC ring.
...
16. Tighten thread/CPU terminology across docs and doc comments: "software thread" for what
...
17. Topology-aware pinning and lCPU terminology: discover the CPU sharing tree at runtime and
...
18. Rebase `web-claude-tweaks` onto post-0.22.0 `main`. It rewrites an already-published
    bookmark (needs approval) and its arbitrary `0.21.0-b` version needs replacing; owed from
    the 0.22.0 close-out plan
19. Unit scaling in report columns (`us`/`ms`): per-row auto-scale so columns stay
...
20. Machine-readable report output (`--format json`, or key=value lines to stay
...
21. Trimmed core stats: `mean/stdev p10-p90` report row, additional to (never replacing)
    ...
    [[57]]
22. Find and label the interference crossover: the band where the tail stops measuring the code
...
23. Investigate: suspend gap missing from samples. A 0.13.5 `--no-inhibit` suspend test
...
24. CLAUDE.md governance model (design cogitation) [20]
25. Revisit probe adjustment under the in-interval vs call-to-call split: probes take one call
...
26. Convert `harness` / `Bench` to probe-based measurement. Will likely need inner-loop support
    ...
    does today.
27. Rename app
28. Design an app to measure IIAC perforanace written in Rust[1]
29. `ice-ps-2t-wait`: iceoryx2 pub/sub with blocking waits via `Listener`/`Notifier` events;
    completes the {transport} × {wait policy} matrix cell that compares against `mpsc-2t`
30. Switch ice benches to the loan-based zero-copy send path (`loan_uninit` + `send`), the API
    a perf-sensitive user would use, and closer to iceoryx2's own benchmark method
31. Payload-size sweep for the round-trip benches (8 B / 8 KiB / 1 MiB), makes iceoryx2's
