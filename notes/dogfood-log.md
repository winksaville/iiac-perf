...
Dated entries on what this project is trying and where the agent-files chafed, failed, or got
amended: the evidence base for the family's convergence decisions
([Changing the agent-files](../AGENTS.md#changing-the-agent-files)). Appended, oldest first.
Moved here 2026-08-30 from `custom-family.md` when the family set and the config absorbed that
file's other jobs. The entries predate the set's adoption and keep the vocabulary they were
written in.
...
resolution an entry retires to [done.md](done.md) with its narrative in
...
- 2026-08-02 (`proposed`): prose.md's <=100 wrap got misapplied as ~64 to match older files' look
  ...
  - the application moved into the prose file itself 2026-08-07, and this entry stays until the
    family takes it
...
- 2026-08-05 (`proposed`): a step is a title, and the version and the step number both leave the
  ...
  - the version was a second identifier for a step, and any prose naming one could be invalidated
    ...
    pasted reports that only the decoder entry above can read
  ...
  - drafted before the `measure-reproducibility` rebase so the cycle dogfoods it, per
    ...
    [chores-06](chores/chores-06.md#docs-steps-are-titles-versions-are-stamps)

- 2026-08-05 (`proposed`): experiments move into the local agent-files, and `custom.md` narrows
  to what cannot be family-wide (superseded in scope by the 2026-08-07 entry below, which drops
  ...
  - rationale in [chores-06](chores/chores-06.md#docs-experiment-in-the-local-agent-files)
  - same session's family-convergence findings, kept in chores because the mailbox protocol was
    handle-then-delete, so a message could never be a record:
    - [jj revset primer audit](chores/chores-06.md#jj-revset-primer-audit-2026-08-05): the
      payload's range bullets are wrong, and the error is the framing, not the gloss
    - [convergence measurements and positions](chores/chores-06.md#convergence-measurements-and-positions-2026-08-05)
...
- 2026-08-07 (`proposed`): the family layer splits out of `custom.md`
  ...
  - one consequence needs the family's agreement, not ours: **"Check the mailbox at acquaint" leaves
    the pinned `AGENTS.md`** and lands in the member's messaging conventions. vc-x1 pinned it in the
    20260802 snapshot, so this is a reversal. The argument is that a non-member cannot perform it at
    all, since building the mailbox path needs a member name and a template path they do not have,
    so it is dead text naming concepts they lack rather than a harmless no-op
...
- 2026-08-07 (`proposed`): cycles run on a bookmark, and `custom.md` stops holding rules
  ...
  - the sharpened convention behind it (wink): **intent decides the file, and nothing gates the
    ...
    2026-08-05 entry above framed the same mechanism around experiments, which read as narrower
    than it is
...
- 2026-08-12 (`proposed`): validation runs at every commit, and the notes-only skip goes
  ...
  - rationale in [chores-06](chores/chores-06.md#docs-validate-every-commit)
...
- 2026-08-14 (`proposed`): the semicolon allowance goes, and the agent-files sweep to zero
  ...
  - rationale in
    [chores-07](chores/chores-07.md#docs-semicolons-leave-the-agent-files)
